...
def run_pipeline(config: dict):
    ...
    LOG2FC_CUTOFF        = float(config["LOG2FC_CUTOFF"])
    BASEMEAN_CUTOFF      = float(config["BASEMEAN_CUTOFF"])
    PADJ_CUTOFF          = float(config["PADJ_CUTOFF"])
    AUTO_BIOTYPE_SPLIT   = bool(config["AUTO_BIOTYPE_SPLIT"])
    GENE_NAME_LOOKUP     = bool(config.get("GENE_NAME_LOOKUP", GENE_NAME_LOOKUP))
    SPECIES              = str(config.get("SPECIES", SPECIES))
    RMATS_FDR_CUTOFF     = float(config["RMATS_FDR_CUTOFF"])
    RMATS_PVAL_CUTOFF    = float(config["RMATS_PVAL_CUTOFF"])
    INCLEVEL_DIFF_CUTOFF = float(config["INCLEVEL_DIFF_CUTOFF"])
    USE_FDR              = bool(config["USE_FDR"])
    FIG_DPI              = int(config["FIG_DPI"])
    FIG_FORMAT           = str(config["FIG_FORMAT"])
    FONT_SIZE            = int(config["FONT_SIZE"])
    COLOR_UP             = str(config["COLOR_UP"])
    COLOR_DOWN           = str(config["COLOR_DOWN"])
    COLOR_NS             = str(config["COLOR_NS"])
    INTERACTIVE_PLOTS    = bool(config["INTERACTIVE_PLOTS"])
    DESEQ2_COLS          = dict(config["DESEQ2_COLS"])
    RMATS_COLS           = dict(config["RMATS_COLS"])
